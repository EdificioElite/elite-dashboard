...
- Para publicar una release: mergear el *release PR* que abre `release-please` en `main` (`.github/workflows/release.yml`)
  ...
  - Con `GITHUB_TOKEN`, la CI no corre sobre el release PR (los checks quedan pendientes); un admin lo mergea manualmente (solo toca ficheros de versión: `package.json`, `package-lock.json`, `CHANGELOG.md` y `.release-please-manifest.json`, de bajo riesgo)
...
- Los roles `migrator` / `migrator_dev` y la tabla `schema_migrations` son propios del dashboard. **Los roles y tablas de n8n (`n8nuser`, `n8n`, `contadores`, `facturas`, `facturaelectrica`, `consumos`) son de otro servicio separado: no se tocan ni se usan.**
- Los roles de migración `migrator` / `migrator_dev` son **SUPERUSER**: las migraciones hacen operaciones de superuser (`ALTER TABLE ... OWNER`, `GRANT ... TO otros roles`, `CREATE ROLE`) que un rol de mínimos privilegios no puede hacer de forma fiable. Se usan **solo** en el init-container de migraciones.
...
1. Crear los roles `migrator` (prod) y `migrator_dev` (dev) como superuser (roles cluster-wide):

```sql
CREATE ROLE migrator SUPERUSER LOGIN PASSWORD '<password-fuerte>';
CREATE ROLE migrator_dev SUPERUSER LOGIN PASSWORD '<password-fuerte>';
```
