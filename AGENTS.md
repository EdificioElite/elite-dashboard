...
- Ejecutar con docker: `docker compose -f docker-compose.e2e.yml up -d && docker compose -f docker-compose.e2e.yml --profile test run --rm playwright sh -c "npm ci && npx playwright test --project=readonly --workers=2 && npx playwright test --project=stateful --workers=1"`
- Los specs se reparten en dos proyectos: `readonly` (paralelo, `--workers=2`) y `stateful` (serial, muta estado compartido: forgot-password, password-change, directiva, admin-stateful). No paralelizar `stateful` con nada.
